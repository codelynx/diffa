...
The Core module provides fundamental types for representing file system items and their metadata. These types are used throughout Diffa for snapshot creation and comparison.
...
} catch DiffaError.invalidSnapshot(let reason) {
    print("Invalid snapshot: \(reason)")
} catch DiffaError.fileNotFound(let path) {
