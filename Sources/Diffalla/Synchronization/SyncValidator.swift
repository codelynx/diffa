...
class SyncValidator {
    ...
    /// - Throws: DiffallaError if validation fails
    func validateSync(
        source: URL,
        destination: URL,
            ...
            if !hasSpace {
                ...
                if let attrs = try? fileManager.attributesOfFileSystem(forPath: destination.path),
                   let freeSpace = attrs[.systemFreeSize] as? Int64 {
                    throw DiffallaError.insufficientSpace(required: requiredSpace, available: freeSpace)
                } else {
                    throw DiffallaError.insufficientSpace(required: requiredSpace, available: 0)
                }
            }
        }
    }
    ...
    /// - Throws: DiffallaError if cannot determine available space
    func hasEnoughSpace(at url: URL, requiredBytes: Int64) throws -> Bool {
        do {
            ...
            guard let freeSpace = attrs[.systemFreeSize] as? Int64 else {
                throw DiffallaError.comparisonFailed(reason: "Cannot determine free space at \(url.path)")
            }
