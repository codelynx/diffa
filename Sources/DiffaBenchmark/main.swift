...
Task {
    do {
        ...
        default:
            ...
            print("Usage: diffa-benchmark [small|medium|large|all]")
            exit(1)
        }
...
struct BenchmarkRunner {
    static func runSmallBenchmarks() async throws -> BenchmarkResult {
        ...
        let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent("diffa-bench-small-\(UUID().uuidString)")
    ...
    static func runMediumBenchmarks() async throws -> BenchmarkResult {
        ...
        let generator = DatasetGenerator()
        let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent("diffa-bench-medium-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    static func runLargeBenchmarks() async throws -> BenchmarkResult {
        ...
        let generator = DatasetGenerator()
        let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent("diffa-bench-large-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: tempDir) }
