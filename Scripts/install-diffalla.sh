...
# Universal installer for Diffalla CLI
...
# Configuration
REPO="codelynx/Diffalla"
BINARY_NAME="diffalla"
INSTALL_DIR="/usr/local/bin"

# Functions
print_header() {
    echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║       Diffalla CLI Installer           ║${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
    echo
}
...
install_homebrew_version() {
    ...
    if ! brew tap | grep -q "codelynx/diffalla"; then
...
install_github_release() {
    ...
    if [[ "$OS" == "macos" ]]; then
        if [[ "$ARCH" == "arm64" ]]; then
            BINARY_URL="https://github.com/$REPO/releases/download/$VERSION/diffalla-macos-arm64"
        else
            BINARY_URL="https://github.com/$REPO/releases/download/$VERSION/diffalla-macos-x86_64"
        fi
    else
        if [[ "$ARCH" == "x86_64" ]]; then
            BINARY_URL="https://github.com/$REPO/releases/download/$VERSION/diffalla-linux-x86_64"
        elif [[ "$ARCH" == "aarch64" ]]; then
            BINARY_URL="https://github.com/$REPO/releases/download/$VERSION/diffalla-linux-arm64"
        else
