...
final class SynchronizationTests: XCTestCase {
    ...
    func testResolveErrorThrows() throws {
        ...
        XCTAssertThrowsError(try resolver.resolve(
            ...
            if case DiffaError.comparisonFailed(let reason) = error {
                XCTAssertTrue(reason.contains("Conflict detected"))
                XCTAssertTrue(reason.contains("file.txt"))
            } else {
                XCTFail("Expected DiffaError.comparisonFailed, got \(error)")
            }
        }
    }
    ...
    func testValidateFailsOnMissingSource() throws {
        ...
        XCTAssertThrowsError(try validator.validateSync(source: source, destination: destination, operations: operations)) { error in
            ...
            if case DiffaError.fileNotFound(let path) = error {
    ...
    func testSyncWithInsufficientSpace() async throws {
        ...
        XCTAssertThrowsError(try validator.validateSync(source: source, destination: destination, operations: operations)) { error in
            if case DiffaError.insufficientSpace(let required, let available) = error {
